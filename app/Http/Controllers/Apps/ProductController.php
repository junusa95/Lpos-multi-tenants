...
// \Log::info('Flutter Headers:', $request->headers->all());
    ...
    //    $productsLines = Product::where('product_category_id',$categoryId)->where('status','published')->orderBy('id','desc')->get();

    //    $products = [];
    //     $picture = public_path().'/images/product.jpg';
    //     $user = Auth::user();
    //     $quantity = 0;

    //    foreach($productsLines as $line){

    //         $product = DB::table('shop_products')->where('product_id', $line->id)->where('shop_id', $shopId)->where('active', 'yes')->first();
    //         if ($product) {
    //             if ($line->image != null){
    //                 $picture = '/images/companies/'.$user->company->folder.'/products/'.$line->image;
    //             }
    ...
    public function product_history(Request $request){
            ...
            // $availableQ = DB::table('shop_products')->where('shop_id',$shop_id)->where('product_id',$product_id)->where('active','yes')->sum('quantity');
            ...
            //         DB::raw(value: 'DATE(updated_at) as date, SUM(added_quantity) as quantity')
            //     ])
            //     ->where('company_id',Auth::user()->company_id)->whereBetween('updated_at', [Carbon::parse($fromdate),Carbon::parse($todate)])
    ...
    // $results = collect(); // initialize collection
    // $runningBalance = $availableQ;
    ...
    //     $results->push((object)$daily); // store as object in collection
    ...
    // // optional: reverse to start from $fromdate
